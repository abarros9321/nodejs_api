    ...
    it('deberia crear una nota', function(done) {
      ...
      request
        ...
        .end(function(err, res) {
          ...
          // Propiedades
			expect(nota).to.have.property('title', '#node-pro');
...
describe('GET', function() {
	it('Obtener nota existente', function(done) {
        ...
        idNota = res.body.nota.id;

        return request.get('/notas/' + idNota)
          .set('Accept', 'application/json')
          .send()
          .expect(200)
          .expect('Content-Type', /application\/json/)
		...
		expect(nota).to.have.property('type', 'js');
		expect(nota).to.have.property('body', 'soy el cuerpo de json');
		expect(nota).to.have.property('id',idNota);
        done();
      }, done);

//Async waterfall for test
      /*async.waterfall([
				function crearNota(cb) {
					request
        			.post('/notas')
        			.set('Accept', 'application/json')
        			.send(data)
